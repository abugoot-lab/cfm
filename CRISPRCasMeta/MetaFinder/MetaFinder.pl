...
######################################################################################
#  MetaFinder, a metagenomic version of CRISRCasFinder 4.2.17, includes a portable version,
...
#
#  MetaFinder and CRISPRCasFinder are distributed under the terms of the GNU General Public License
...
my $rcfowce = 0; # option allowing to run CasFinder only when any CRISPR exists (default value=0) (set if -cas is set) ##NV not used for metagenome
...
my $definition = "General"; # option allowing to specify CasFinder definition (if option -cas is set) to be more or less stringent (default value="SubTyping"). Other options include "Typing" and "General". 
...
##NV 14/06/2018 multifasta Cas systems analysis, see end of the while loop for Cas Finder
my ($casfile, %jsonCAS, @tabCRISPRCasClustersA);
$casfile = "";
my $nbrCas = 0;
my %contigRes;
my @contigs =[];
my $jsonLineSummary = $jsonLineRes; ##NV 20/06/2018
open (JSONSUM, ">summary_Result.json") or die "open : $!"; ##NV 20/06/2018
print JSONSUM "{\n";
##NV 14/06/2018
...
while($seq = $seqIO->next_seq()){  # DC - replace 'next_seq' by 'next_seq()'
  ...
  #my @seqIdglobal = split(/\./,$seqID1); ##NV 14/06/2018
  #push @contigs, $seqIdglobal[0]; ##NV 14/06/2018
  push @contigs, $seqID1;##NV 12/07/2018
  ...
  if ($seqLength >= $seqMinSize){
	...
	#$seqID1 = $seqID1."_".$inputfileCount;
  	...
  	# DC - 06/2017 - Integration of Cas analysis ##NV modified for multi fasta treatment 14/06/2018
  	#my ($casfile, $jsonCAS, @tabCRISPRCasClustersA);
  	#$casfile = "";
  	#my $nbrCas = 0;
	...
	#	($nbrCas, $casfile, $jsonCAS, @tabCRISPRCasClustersA) = casFinder($ResultDir,$inputfile,$seqDesc,$RefSeq,$nbrcris,$kingdom,$casfinder);
  	...
  	#if(! $nbrCas){ $nbrCas = 0; }
	## NV modified for multi fasta treatment 14/06/2018
	...
	##CCF 4.2.17 last version before metagenome
	...
	#get information from @tabCRISPRCasClustersA
  	...
  	#$jsonLineRes .= "{\n\"Id\":\"".$RefSeq."\",\n\"Description\":\"".$seqDesc."\",\n\"AT\":".$globalAT.",\n\"Length\":".$seqLength.",\n\"Summary_CRISPR-Cas\":\"".$tempSumDataCcC."\",\n\"Crisprs\":".$catJSONcrispr.",\n\"Cas\":".$catJSONcas."\n},\n"; ##from CCF 4.2.17 last version before metagenome adaptation
	$contigRes{$RefSeq} = "{\n\"Id\":\"".$RefSeq."\",\n\"Description\":\"".$seqDesc."\",\n\"AT\":".$globalAT.",\n\"Length\":".$seqLength.",\n\"Summary_CRISPR-Cas\":\"".$tempSumDataCcC."\",\n\"Crisprs\":".$catJSONcrispr.",\n\"Cas\":";
...
##NV 14/06/2018 multifasta Cas systems analysis, see start of while loop for attributes initiation
$inputfile = $userfile;
#print "\n\nResultDir : $ResultDir\ninputfile : $inputfile\n";
($nbrCas, $casfile, %jsonCAS, @tabCRISPRCasClustersA) = casFinder($ResultDir,$inputfile,"Prodigal_Res",$kingdom,$casfinder);
#print "\n\nSystem nbr ".$nbrCas."\n\n";
$nbrAllCas = $nbrCas;
foreach my $contig (@contigs) {
	if(exists($contigRes{$contig})) {
		if (exists($jsonCAS{$contig})) {
			$jsonLineRes .= $contigRes{$contig}."[".$jsonCAS{$contig}."]\n\n},\n";
			$jsonLineSummary .= $contigRes{$contig}."[".$jsonCAS{$contig}."]\n\n},\n"; ##NV 20/06/2018
		}
		else{
			$jsonLineRes .= $contigRes{$contig}."[]\n},\n";
		}
	}
}
##NV 14/06/2018
##NV 20/06/2018
# End and close file JSONSUM
$jsonLineSummary .= "]\n";
$jsonLineSummary =~ s/,\n]/\n]/; # change end of file to better fit JSON format
$jsonLineSummary .= "}\n";

print JSONSUM $jsonLineSummary;
close (JSONSUM);
# move JSONSUM in ResultDir
if(-e "summary_Result.json"){
  makesystemcall("mv summary_Result.json $ResultDir");
}
##NV 20/06/2018
...
sub casFinder
{
  #my($ResultDir,$inputfile,$seqDesc,$RefSeq,$nbrcris,$kingdom,$casfinder)=@_; ##CCF 4.2.17 last version before metagenome
  my($ResultDir,$inputfile,$RefSeq,$kingdom,$casfinder)=@_;

  my $repProkka = "";
  if($useProkka){ $repProkka = $ResultDir."/prokka_".$RefSeq; }
  else{ $repProkka = $ResultDir."/".$RefSeq; } ##NV 15/06/2018
  #else{ $repProkka = $ResultDir."/prodigal_".$RefSeq; } ##CCF 4.2.17 last version before metagenome

  my $casDir =  $ResultDir."/casfinder_".$RefSeq."/"; 
 
  #my $jsonCAS = ""; # variable allowing to write JSON file related to Cas ##CCF 4.2.7 last version before metagenome
  my %jsonCAS;
  ...
  eval{
    ...
    if ( (-d $casfinder) and (-d $casdb) and (-d $profiles) ) {
	  #if($default){
	    $macsyfinder = "macsyfinder -w $cpuMacSyFinder -d $casdb -p $profiles --sequence-db $proteome all --out-dir $casDir --i-evalue-select 0.01 "; 
    ...
    if (-e $json){ 
		...
		my %hashGeneOther=(); #Hash table for sequence Other
			...
			while (defined ($line = <GFF>)){
				...
				#my @partIDs = split(/_/, $v); ## CCF 4.2.17 last version
				#my $elemIDpart = "ID=1_".$partIDs[$#partIDs].";"; ## CCF 4.2.17 last version
				my @partIDs = split(/_/, $v); ##NV 14/06/2018
				my $elemIDpart = "_".$partIDs[$#partIDs].";"; ##NV 14/06/2018
				#my @contigID = split(/\t/, $line);
				pop @partIDs; ##NV 14/06/2018
				my $contigRef = join("_",@partIDs); ##NV 14/06/2018
				    if($useProdigal){
					#if ( $line =~ /$elemIDpart/ && $line !~ /#/ ) { ## CCF 4.2.17 last version
					if ( $line =~ /$elemIDpart/ && $line !~ /#/ &&  $line =~ /$contigRef/) { ##NV 14/06/2018
			...
			#foreach my $v (@tabGenes) {
			#	print"\n\nSequence : $v\nType : $hashGeneType{$v}\nLocation : $hashGeneBegin{$v}, $hashGeneEnd{$v}\nStrand : $hashGeneStrand{$v}\nOther : $hashGeneOther{$v}";
			#}
			#print"\n";
			...
			my $jsonLineCas = ""; ##NV 18/06/2018
			my %jsonHash; ##NV 18/06/2018
			my %contLoc; ##NV 18/06/2018
			#my %endSystem; ##NV 18/06/2018
  		
			#open(JSONCAS,">",$ResultDir."/".$RefSeq."_cas.json") or die("Could not open the JSON file $RefSeq _cas.json because $!\n");
			#print JSONCAS "[";
			#$jsonCAS = "["; ##CCF 4.2.17 last version before metagenome
				...
				for (my $j = 0; $j < $sizeGenes; $j++) {
					...
					if ( defined($matcher) ){
						...
						if(-e $inputfileSeq){
							...
							my @clear = split(/\_/,$seqName); ##NV 09/07/2018
							pop @clear; ##NV 09/07/2018
							my $newSeqName = join("_",@clear); ##NV 09/07/2018
							#my @seqId = split(/\./,$newSeqName); ##NV 15/06/2018
							#$inputfileSeq = $ResultDir."/analyzedSequences/".$seqId[0].".fna"; ##NV 15/06/2018
							$inputfileSeq = $ResultDir."/analyzedSequences/".$newSeqName.".fna"; ##NV 12/07/2018
							#$inputfileSeq = $RefSeq.".fna"; ##CCF 4.2.17 last version before metagenome
							if(-e $inputfileSeq){
								my $casSeqFile = $ResultDir."/rawCas.fna"; 
								open (RAWCAS, ">>$casSeqFile") or die "open : $!";  #Open casSeqFile

								my $dbSeq = Bio::DB::Fasta->new($inputfileSeq); # sequence file (or $userfile)						
								#my $seqidCas = $seq->id; # ID of sequence ##CCF 4.2.17 last version before metagenome
								my @contigId = split(/\_/,$seqName); ##NV 15/06/2018
								pop @contigId; ##NV 15/06/2018
								my $seqidCas = join("_",@contigId); ##NV 15/06/2018
								# DC - 11/2017 - get raw sequence of Cas
								...
								##NV 18/06/2018 for json format
								#my @globalId = split(/\./,$seqidCas);##NV 18/06/2018 $globalId[0] replaced by $seqidCas
								my $contigLine = $systemName."|".$matcher."|".$hashGeneBegin{$seqName}."|".$hashGeneEnd{$seqName}."|".$hashGeneStrand{$seqName};
								if (exists $jsonHash{$seqidCas}){
									push @{$jsonHash{$seqidCas}}, $contigLine; ##NV 17/07/2018
									#if ($contLoc{$globalId[0]}[0] > $hashGeneBegin{$seqName} ){
									#	$contLoc{$globalId[0]}[0] = $hashGeneBegin{$seqName};
									#}
									#if ($contLoc{$globalId[0]}[1] < $hashGeneEnd{$seqName} ) {
									#	$contLoc{$globalId[0]}[1] = $hashGeneEnd{$seqName};
									#}
								}
								else{
									$jsonHash{$seqidCas} = [$contigLine];
									#$contLoc{$globalId[0]} = [$hashGeneBegin{$seqName},$hashGeneEnd{$seqName}];
								}
								
								if (exists $contLoc{$seqidCas."|".$systemName}){
									if ($contLoc{$seqidCas."|".$systemName}[0] > $hashGeneBegin{$seqName} ){
										$contLoc{$seqidCas."|".$systemName}[0] = $hashGeneBegin{$seqName};
									}
									if ($contLoc{$seqidCas."|".$systemName}[1] < $hashGeneEnd{$seqName} ) {
										$contLoc{$seqidCas."|".$systemName}[1] = $hashGeneEnd{$seqName};
									}
								}
								else {
									$contLoc{$seqidCas."|".$systemName} = [$hashGeneBegin{$seqName},$hashGeneEnd{$seqName}];
								}
								##NV 18/06/2018 End for json format
			...
			$nbCas = 0;
			##NV 18/06/2018 for json format
			foreach my $jsonLineKey (keys %jsonHash) {
			
				my %systemCas;
				foreach my $contigCas (@{$jsonHash{$jsonLineKey}}) {
					#print "\n\nContig : $jsonLineKey \nResult : $contigCas";
					my @arrayContig = split(/\|/,$contigCas);
					if (!exists $systemCas{$arrayContig[0]}) {
						$systemCas{$arrayContig[0]} = "{\n\"Type\": \"".$arrayContig[0]."\",\n\"Start\": ".$contLoc{$jsonLineKey."|".$arrayContig[0]}[0].",\n\"End\": ".$contLoc{$jsonLineKey."|".$arrayContig[0]}[1].",\n\"Genes\": [\n";
						$systemCas{$arrayContig[0]} .= "{\n\"Sub_type\": \"".$arrayContig[1]."\",\n\"Start\": ".$arrayContig[2].",\n\"End\": ".$arrayContig[3].",\n\"Orientation\": \"".$arrayContig[4]."\"\n}";
					}
					else {
						$systemCas{$arrayContig[0]} .= ",\n{\n\"Sub_type\": \"".$arrayContig[1]."\",\n\"Start\": ".$arrayContig[2].",\n\"End\": ".$arrayContig[3].",\n\"Orientation\": \"".$arrayContig[4]."\"\n}";
					}
				...
				#$jsonCAS{$jsonLineKey} = "";
				foreach my $system (sort (keys %systemCas)) {
					$nbCas += 1;
					print "\n\n".$systemCas{$system}."\n\n";
					if (!exists $jsonCAS{$jsonLineKey}) {
						$jsonCAS{$jsonLineKey} = $systemCas{$system};
					}
					else {
						$jsonCAS{$jsonLineKey} .= "]},\n".$systemCas{$system};
					}
